...
Dentro del archivo <strong>server.php</strong> se encuentra un apartado que se encarga de conectar con la base de datos, el cual contiene la ruta del ‘localhost’, ‘usuario’, ‘contraseña’, ‘nombre_base_de_datos’. <strong>Estos últimos 3 campos pueden ser modificados dependiendo la configuración de cada usuario</strong> sin embargo en este ejercicio el único que debe conservar su nombre tal cual es el campo de Prueba Selenium ya que el programa creara una BD con ese nombre a la cual accederá.
